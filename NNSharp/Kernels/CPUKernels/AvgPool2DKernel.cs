...
{
    [Serializable()]
    public class AvgPool2DKernel : IKernel
    {
        public void Execute()
        {
                    ...
                    for (int w = 0; w < dimO.w; ++w)
                    {
                        for (int h = 0; h < dimO.h; ++h)
                        {
                            ...
                            for (int idxH = stH; idxH < stH + kernelDim.w; ++idxH)
                            {
                                ...
                                {
                                    output[h, w, channel, batch] += input[idxV, idxH, channel, batch];
                                }
                            }

                            output[h, w, channel, batch] = output[h, w, channel, batch] / (kernelDim.w * kernelDim.h);
                        }
                    }
                }
            }
