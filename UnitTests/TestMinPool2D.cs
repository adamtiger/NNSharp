...
{
    [TestClass]
    public class TestMinPool2D
    {
        [TestMethod]
        public void Test_MinPool2D_Execute()
        {
            // Initialize data.
            ...
            MinPool2DLayer pool = new MinPool2DLayer(0, 0, 1, 1, 2, 2);
            pool.SetInput(data);
            pool.Execute();
            ...
            Assert.AreEqual(output[1, 0, 0, 0], 0.0, 0.0000001);
            Assert.AreEqual(output[0, 1, 0, 0], 2.0, 0.0000001);
            Assert.AreEqual(output[1, 1, 0, 0], 0.0, 0.0000001);

            Assert.AreEqual(output[0, 0, 1, 0], 0.0, 0.0000001);
            Assert.AreEqual(output[1, 0, 1, 0], -1.0, 0.0000001);
